...
#define MULTILINE_RESOURCE(...) #__VA_ARGS__

    static std::string loadResource()
    {
        const char *data = MULTILINE_RESOURCE(
/*
                               ...
                               global float3 *a_out, global float3 *b_out, global float3 *n_out, global float *ir_out)
{
  ...
  p0x_sin = -sincos((float3)(PHASE_IN_RAD0, PHASE_IN_RAD1, PHASE_IN_RAD2) + p0.x, &p0x_cos);
  p0y_sin = -sincos((float3)(PHASE_IN_RAD0, PHASE_IN_RAD1, PHASE_IN_RAD2) + p0.y, &p0y_cos);
  p0z_sin = -sincos((float3)(PHASE_IN_RAD0, PHASE_IN_RAD1, PHASE_IN_RAD2) + p0.z, &p0z_cos);
  ...
  float3 a = (float3)(dot(v0, p0x_cos),
                      dot(v1, p0y_cos),
                      dot(v2, p0z_cos)) * (float3)(AB_MULTIPLIER_PER_FRQ0, AB_MULTIPLIER_PER_FRQ1, AB_MULTIPLIER_PER_FRQ2);
  float3 b = (float3)(dot(v0, p0x_sin),
                      dot(v1, p0y_sin),
                      dot(v2, p0z_sin)) * (float3)(AB_MULTIPLIER_PER_FRQ0, AB_MULTIPLIER_PER_FRQ1, AB_MULTIPLIER_PER_FRQ2);
