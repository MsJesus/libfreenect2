...
DumpDepthPacketProcessor::DumpDepthPacketProcessor()
  : impl_(new DumpDepthPacketProcessorImpl()) {
}
...
void DumpDepthPacketProcessor::loadP0TablesFromCommandResponse(unsigned char* buffer, size_t buffer_length) {
  impl_->p0table_ = new unsigned char[buffer_length];
  std::memcpy(impl_->p0table_, buffer, buffer_length);
}

void DumpDepthPacketProcessor::loadXZTables(const float *xtable, const float *ztable) {
  impl_->xtable_ = new float[TABLE_SIZE];
  std::memcpy(impl_->xtable_, xtable, TABLE_SIZE * sizeof(float));

  impl_->ztable_ = new float[TABLE_SIZE];
  std::memcpy(impl_->ztable_, ztable, TABLE_SIZE * sizeof(float));
}

void DumpDepthPacketProcessor::loadLookupTable(const short *lut) {
  impl_->lut_ = new short[LUT_SIZE];
  std::memcpy(impl_->lut_, lut, LUT_SIZE * sizeof(short));
}
} /* namespace libfreenect2 */
